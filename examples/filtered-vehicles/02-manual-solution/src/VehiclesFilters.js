...
class VehiclesFilters extends React.Component {
  ...
  _fetchData() {
    const { cacheParams, dispatch, query } = this.props;
    const { make } = query;

    if (JSON.stringify({ make }) !== JSON.stringify(cacheParams)) {
      dispatch(actions.fetchModels(make));
    }
  }
...
export default compose(
  withVehiclesRouter,
  connect(state => ({
    cacheParams: state.modelsCacheParams,
    models: state.models,
  })),
)(VehiclesFilters);
